...
class Juno(object):
    ...
    def request(self, request, method='*', **kwargs):
        ...
        _response.response = JunoResponse()
        ...
        for route in self.routes:
            ...
            if response is None: response = _response.response
...
_response = threading.local()

def append(body):
    """Add text to response body. """
    return _response.response.append(body)

def header(key, value):
    """Set a response header. """
    return _response.response.header(key, value)
...
def status(code):
    _response.response.config['status'] = code
...
def redirect(url, code=302):
    ...
    _response.response.config['headers'] = { 'Location': url }
...
def servererror(error='Unspecified error', file=None):
    ...
    _response.response.config['body'] = ''
    return template(file, error=error)
